...
local function RippleEffect(object)
    spawn(function()
        ...
        wait(0.7)
        TweenService:Create(Ripple, TweenInfo.new(1, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {ImageTransparency = 0}):Play()
        wait(0.5)
        Ripple:Destroy()
    end)
end
...
function library:Window(name,game)
    ...
    function Tabs:Tab(text)
        ...
        function TabElements:Button(text,callback)
            ...
            NameButton.Size = UDim2.new(0, 476, 0, 32)
            NameButton.ZIndex = 5
        ...
        function TabElements:MutiDropdown(name, options, callback)
            ...
            NameDropdown.Name = (name.."NameDropdown")
            NameDropdown.Parent = SectionContent
            NameDropdown.BackgroundColor3 = Color3.fromRGB(45, 45, 45)
            NameDropdown.Position = UDim2.new(0.020833334, 0, 0, 0)
            NameDropdown.Size = UDim2.new(0, 475, 0, 30)
            NameDropdown.ZIndex = 5
        ...
        function TabElements:DestroyGui()
            ...
            ButtonRounded.Size = UDim2.new(1, 0, 1, 0)
            ButtonRounded.ZIndex = 5
            ButtonRounded.Image = "rbxassetid://"
            ButtonRounded.ImageColor3 = Color3.fromRGB(255, 75, 75)
            ButtonRounded.ImageTransparency = 1.000
            ButtonRounded.ScaleType = Enum.ScaleType.Slice
            ButtonRounded.SliceCenter = Rect.new(100, 100, 100, 100)
...
spawn(function()
    while task.wait() do
        pcall(function()
            if _G.AutoFarm then
                haki()
                antikb(game.Players.LocalPlayer.Character.HumanoidRootPart)
                noclip(game.Players.LocalPlayer.Character.HumanoidRootPart)
                autofarm()
            else
                pcall(function()
                    for a, e in pairs(game.Players.LocalPlayer.Character:GetChildren()) do
                        if e:IsA("BasePart") then
                            e.CanCollide = true
                        end
                    end
                    game.Players.LocalPlayer.Character.HumanoidRootPart:FindFirstChild("AntiKnockback"):Destroy()
                end)
            end
        end)
    end
...
if game.PlaceId == 6329844902 then
    LastPirateworld1 = true
end

function checkquest()
    local level = game:GetService("Players")["LocalPlayer"].PlayerStats.Level.Value
    if LastPirateworld1 then
        if level == 1 or level < 15 then
            Mob = "Bandit [Lv:5]"
            Quest = "Bandit"
            LctQuest = CFrame.new(228.19596862793, 26.931638717651, -1563.6461181641)
            LctSafe = LctQuest
        elseif level == 15 or level < 30 then
            Mob = "Pirates [Lv:15]"
            Quest = "Pirates"
            LctQuest = CFrame.new(11.938439369202, 26.671655654907, -1658.6362304688)
            LctSafe = LctQuest
        elseif level == 30 or level < 60 then
            Mob = "BagyPirates [Lv:30]"
            Quest = "BagyPirates"
            LctQuest = CFrame.new(1635.9267578125, 30.821800231934, 504.79016113281)
            LctSafe = LctQuest
        elseif level == 60 or level < 150 then
            Mob = "Clown Pirate [Lv:60]"
            Quest = "Clown Pirate"
            LctQuest = CFrame.new(1886.5905761719, 30.821800231934, 563.58935546875)
            LctSafe = LctQuest
        elseif level == 150 or level < 200 then
            Mob = "Revolutionary Troop [Lv:150]"
            Quest = "Revolutionary Troop"
            LctQuest = CFrame.new(-2260.6110839844, 17.310565948486, -2625.8486328125)
            LctSafe = LctQuest
        elseif level == 200 or level < 600 then
            Mob = "Marines [Lv:200]"
            Quest = "Marines"
            LctQuest = CFrame.new(-2725.3415527344, 14.687033653259, 430.93704223633)
            LctSafe = LctQuest
        elseif level == 600 or level < 700 then
            Mob = "Fishman [Lv:600]"
            Quest = "Fishman"
            LctQuest = CFrame.new(4875.4204101562, 31.320676803589, 2384.5993652344)
            LctSafe = LctQuest
        elseif level == 700 or level < 850 then
            Mob = "WinterBandit [Lv:700]"
            Quest = "WinterBandit"
            LctQuest = CFrame.new(2488.6096191406, 18.879720687866, 4665.0375976562)
            LctSafe = LctQuest
        elseif level == 850 or level < 1100 then
            Mob = "Sky Bandit [Lv:850]"
            Quest = "Sky Bandit"
            LctQuest = CFrame.new(7.3945937156677, 251.56329345703, 3551.2741699219)
            LctSafe = LctQuest
        elseif level == 1100 or level < 1400 then
            Mob = "Monkey [Lv:1150]"
            Quest = "Monkey"
            LctQuest = CFrame.new(3588.9555664062, 34.900012969971, -2885.1767578125)
            LctSafe = LctQuest
        elseif level > 1399 then
            Mob = "Skeleton [Lv:1400]"
            Quest = "Skeleton"
            LctQuest = CFrame.new(-6788.4697265625, 53.127319335938, 3659.5388183594)
            LctSafe = LctQuest
        end
    end
end
...
function antikb(objects)
    if objects:FindFirstChild("AntiKnockback") then
    else
        local vc = Instance.new("BodyVelocity", objects)
        vc.Name = "AntiKnockback"
        vc.MaxForce = Vector3.new(1, 1, 1) * math.huge
        vc.Velocity = Vector3.new(0, 0, 0)
    end
end

function noclip(objects)
    for a, e in pairs(objects:GetChildren()) do
        if e:IsA("BasePart") then
            e.CanCollide = false
        end
    end
...
function haki()
    if not game.Players.LocalPlayer.Character:FindFirstChild("LHakiArm") and not game.Players.LocalPlayer.Character:FindFirstChild("RHakiArm") then
        game:GetService("ReplicatedStorage").Haki:FireServer("Buso")
    end
end

function equip(tool)
    local Tool = game.Players.LocalPlayer.Backpack:FindFirstChild(tostring(tool))
    if Tool then
...
function autofarm()
    if game:GetService("Players")["LocalPlayer"].Quest.Doing.Value == "None" then
        checkquest()
        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = LctQuest
        wait(1)
        game:GetService("ReplicatedStorage").FuncQuest:InvokeServer(Quest)
    else
        checkquest()
        if game.Workspace:FindFirstChild(Mob) then
            for i, v in pairs(game.Workspace:GetChildren()) do
                checkquest()
                if v:FindFirstChild("Humanoid") then
                    if v.Name == Mob then
                        if string.find(game:GetService("Players")["LocalPlayer"].Quest.Doing.Value, Quest) then
                            if v.Humanoid.Health > 0 then
                                game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.HumanoidRootPart.CFrame * CFrame.new(0, 7.8, 0) * CFrame.Angles(math.rad(-90), 0, 0)
                            ...
                            game:GetService("Players")["LocalPlayer"].Quest.Doing.Value = "None"
                        end
                    end
                end
            end
        elseif game.Workspace.Lives:FindFirstChild(Mob) then
            for i, v in pairs(game.Workspace.Lives:GetChildren()) do
                ...
                if v:FindFirstChild("Humanoid") then
                    if v.Name == Mob then
                        if string.find(game:GetService("Players")["LocalPlayer"].Quest.Doing.Value, Quest) then
                            if v.Humanoid.Health > 0 then
                                game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.HumanoidRootPart.CFrame * CFrame.new(0, 7.5, 0) * CFrame.Angles(math.rad(-90), 0, 0)
                                equip(_G.SelectWeapon)
                                game:GetService("VirtualUser"):CaptureController()
                                game:GetService("VirtualUser"):Button1Down(Vector2.new(1280, 672))
                            else
            ...
            checkquest()
            game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = LctSafe * CFrame.new(0, 50, 0)
        end
    end
end
...
local Target = game:GetService("ReplicatedStorage").okStats
StatsTab:Toggle("Melee", _G.Melee, function(v)
    _G.Melee = v
end)
spawn(function()
    while task.wait() do
        if _G.Melee then
            pcall(function()
                Target:FireServer(point, "1")
            end)
        end
    end
end)
StatsTab:Toggle("Sword", _G.Sword, function(v)
    _G.Sword = v
end)
spawn(function()
    while task.wait() do
        if _G.Sword then
            pcall(function()
                Target:FireServer(point, "2")
            end)
        end
    end
end)
StatsTab:Toggle("Defense", _G.Defense, function(v)
    _G.Defense = v
end)
spawn(function()
    while task.wait() do
        if _G.Defense then
            pcall(function()
                Target:FireServer(point, "3")
            end)
        end
    end
end)
StatsTab:Toggle("Devil Fruit", _G.Fruit, function(v)
    _G.Fruit = v
end)
spawn(function()
    while task.wait() do
        if _G.Fruit then
            pcall(function()
                Target:FireServer(point, "4")
            end)
        end
    end
end)

StatsTab:Slider("Point",1,10, 1,nil,function(v)
    point = v
end)
